...
md`# D3 com Crossfilter e DC.js e Leaflet (Crimes em chicago)`
)});
...
integrity="sha512-xwE/Az9zrjBIphAcBb3F6JVqxf46+CDLwfLMHloNu6KEQCAWi6HcDUbeOfBIptF7tcCzusKFjFw2yuvEpDL9wQ==" crossorigin=""/>`
  ...
  main.variable(observer("buildvis")).define("buildvis", ["md","container","dc","d3","dayDimension","primaryTypeDimension","primaryTypeCountGroup","dayTypeHomicideCountGroup","dayTypeRobberyCountGroup","dayTypeBurglaryCountGroup"], function(md,container,dc,d3,dayDimension,primaryTypeDimension,primaryTypeCountGroup,dayTypeHomicideCountGroup,dayTypeRobberyCountGroup,dayTypeBurglaryCountGroup)
{
  let view = md`${container()}`;
  
     let barChart = dc.barChart(view.querySelector('#type-chart'));
     let compositeChart = dc.compositeChart(view.querySelector("#day-chart"))
  

     let xScale = d3.scaleTime()
                  .domain([dayDimension.bottom(1)[0].dtg, dayDimension.top(1)[0].dtg])
    ...
    compositeChart.width(500)
                  ...
                  .compose([
                    ...
                    .group(dayTypeHomicideCountGroup,'HOMICIDE')
                    .ordinalColors(['#ca0020']),
                    dc.lineChart(compositeChart)
                    .group(dayTypeRobberyCountGroup,'ROBBERY')
                    .ordinalColors(['#0571b0']),
                    dc.lineChart(compositeChart)
                    .group(dayTypeBurglaryCountGroup,'BURGLARY')
  ...
  main.variable(observer()).define(["dayDimension"], function(dayDimension){return(
dayDimension.bottom(1)[0]
)});
  main.variable(observer("map")).define("map", ["buildvis","L"], function(buildvis,L)
{
  buildvis;
  let mapInstance = L.map('mapid').setView([41.878113,-87.629799], 9.5)
  ...
  main.variable(observer("circles")).define("circles", ["circlesLayer","dataset","L"], function(circlesLayer,dataset,L)
{
    ...
    dataset.forEach( function(d) {
      ...
      color: color_json[d['Primary Type']],
      weight: 2,
      fillColor: color_json[d['Primary Type']],
      fillOpacity: 0.5
      })
...
<main role="main" class="container">
   ...
   <p>Crime data via <a href="https://data.cityofchicago.org/Public-Safety/Crimes-2001-to-present/ijzp8t2">Chicago data portal</a>.</p>
  </main>
 `
}
)});
...
facts.dimension(d => d3.timeDay(d.dtg))
  ...
  main.variable(observer("dayCountGroup")).define("dayCountGroup", ["dayDimension"], function(dayDimension){return(
dayDimension.group()
)});
  main.variable(observer("dayTypeRobberyCountGroup")).define("dayTypeRobberyCountGroup", ["dayDimension"], function(dayDimension){return(
dayDimension.group().reduceSum(d => d['Primary Type'] == 'ROBBERY')
)});
  main.variable(observer("dayTypeHomicideCountGroup")).define("dayTypeHomicideCountGroup", ["dayDimension"], function(dayDimension){return(
dayDimension.group().reduceSum(d => d['Primary Type'] == 'HOMICIDE')
)});
  main.variable(observer("dayTypeBurglaryCountGroup")).define("dayTypeBurglaryCountGroup", ["dayDimension"], function(dayDimension){return(
dayDimension.group().reduceSum(d => d['Primary Type'] == 'BURGLARY')
)});
...
primaryTypeDimension.group()
  ...
  main.variable(observer()).define(["dayTypeHomicideCountGroup"], function(dayTypeHomicideCountGroup){return(
dayTypeHomicideCountGroup.all()
)});
  main.variable(observer()).define(["dayTypeRobberyCountGroup"], function(dayTypeRobberyCountGroup){return(
dayTypeRobberyCountGroup.all()
)});
  main.variable(observer()).define(["dayTypeBurglaryCountGroup"], function(dayTypeBurglaryCountGroup){return(
dayTypeBurglaryCountGroup.all()
)});
