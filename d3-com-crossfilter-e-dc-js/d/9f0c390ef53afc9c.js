// https://observablehq.com/@igorchavesmoura/d3-com-crossfilter-e-dc-js@297
...
d3.csv("https://gist.githubusercontent.com/emanueles/d8df8d875edda71aa2e2365fae2ce225/raw/1e949d3da02ed6caa21fe3a7a12a4e5a611a4bab/stocks.csv").then(function(data){
  ...
  data.forEach(function(d,i){
       ...
       if(i != 0){
         ...
         d.googlePerc = (d.google - first.google)/first.google;
         d.facebookPerc = (d.facebook - first.facebook)/first.facebook;
  ...
  main.variable(observer("buildcomposite")).define("buildcomposite", ["md","container","dc","d3","dateDim","googleByDayGroup","fbByDayGroup"], function(md,container,dc,d3,dateDim,googleByDayGroup,fbByDayGroup)
{
  ...
  main.variable(observer("googleByDayGroupPercentage")).define("googleByDayGroupPercentage", ["dateDim"], function(dateDim){return(
dateDim.group(d => d).reduceSum(d => d.googlePerc)
)});
  main.variable(observer()).define(["googleByDayGroupPercentage"], function(googleByDayGroupPercentage){return(
googleByDayGroupPercentage.all()
)});
  main.variable(observer("facebookByDayGroupPercentage")).define("facebookByDayGroupPercentage", ["dateDim"], function(dateDim){return(
dateDim.group(d => d).reduceSum(d => d.facebookPerc)
)});
  main.variable(observer("acoes")).define("acoes", ["md","container","dc","d3","dateDim","googleByDayGroupPercentage","facebookByDayGroupPercentage"], function(md,container,dc,d3,dateDim,googleByDayGroupPercentage,facebookByDayGroupPercentage)
{
  ...
  compositeChart.width(800)
              ...
              .compose([
                  dc.lineChart(compositeChart)
                    .group(googleByDayGroupPercentage, 'Google')
                    .ordinalColors(['steelblue']),
                  dc.lineChart(compositeChart)
                    .group(facebookByDayGroupPercentage, 'Facebook')
                    .ordinalColors(['darkorange'])])
  dc.renderAll()
  return view      
}
...
crossfilter(datasetMovies)
  ...
  main.variable(observer("yearDim")).define("yearDim", ["factsMovie"], function(factsMovie){return(
factsMovie.dimension(d => d.Year)
)});
  main.variable(observer("genreDim")).define("genreDim", ["factsMovie"], function(factsMovie){return(
factsMovie.dimension(d => d.Genre)
)});
  main.variable(observer("genreGrossGroup")).define("genreGrossGroup", ["genreDim"], function(genreDim){return(
genreDim.group().reduceSum(d => d.Worldwide_Gross_M)
)});
  main.variable(observer("yearGrossGroup")).define("yearGrossGroup", ["yearDim"], function(yearDim){return(
yearDim.group().reduceSum(d => d.Worldwide_Gross_M)
)});
  main.variable(observer("by_year")).define("by_year", ["md","container","dc","yearDim","yearGrossGroup","d3"], function(md,container,dc,yearDim,yearGrossGroup,d3)
{
  let view = md`${container('chartByYear','Bilheteria total por Ano (em milhões de dólares)')}`
  ...
  main.variable(observer("by_genre")).define("by_genre", ["md","container","dc","genreDim","genreGrossGroup","d3"], function(md,container,dc,genreDim,genreGrossGroup,d3)
{
  let view = md`${container('chartByGenre','Bilheteria total por Gênero (em milhões de dólares)')}`
