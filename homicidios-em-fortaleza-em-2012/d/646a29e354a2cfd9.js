...
d3.schemeBlues[9]
  ...
  main.variable(observer("colorScale")).define("colorScale", ["d3","homicidesByPopulation","blues"], function(d3,homicidesByPopulation,blues){return(
d3.scaleQuantize()
    .domain(d3.extent(homicidesByPopulation[1],d => d.HomicidiosPorHabitante))
    .range(blues)
)});
...
function style(feature) {
		 return {
					...
					dashArray: '3',
					fillOpacity: 0.6,
					fillColor: colorScale(homicidesByPopulation[0].get(feature.properties.NOME))
				};
	}
)});
...
d3.csv("https://gist.githubusercontent.com/emanueles/8ee225792454a78151d0842a10642a29/raw/9984e32effa99da3cea2ec87a7c9b7b231c71d3c/fortaleza_crimes_populacao.csv").then(function(data){
  ...
  data.forEach(d => {
    ...
    d.HomicidiosPorHabitante = ((d.Homicidios*d.Populacao)/100000)
    
    map.set(d.Bairro,d.HomicidiosPorHabitante);
